...
const calculateTaskProgress = (task: any) => {
    ...
    const completedSteps = task.etapes.filter((step: any) => step.statut === 'TERMINEE').length;
    console.log(`Task: ${task.nom}, Completed Steps: ${completedSteps}, Total Steps: ${task.etapes.length}`);
...
const ProjectDetails: React.FC = () => {
  ...
  const isTravailleur = user?.roles.some(role => role.name === 'TRAVAILLEUR');
                        ...
                        <div className="flex flex-wrap gap-2">
                          {task.travailleurs.map((worker) => (
                            ...
                            >
                              ...
                              <span className="text-sm text-gray-700">{worker.username}</span>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            <div className="space-y-4">
              {project.postes.map((post) => (
                <div key={post.id} className="border rounded-lg p-4">
                  ...
                  {isTravailleur && !post.travailleur && (
                    <div className="mt-4">
